...
function traverseWithScope(
  ...
  switch (node.type) {
    ...
    case NodeType.AssignmentExpression: {
      ...
      // Note: we don't traverse assign.left here because we already handled it above
    ...
    case NodeType.VariableDeclaration: {
      ...
      if (decl.init) traverseWithScope(decl.init, locals, captures, mutableCaptures);
      break;
    }
    case NodeType.FunctionExpression: {
      ...
      // Recurse
      traverseWithScope(func.body, newLocals, nestedCaptures, nestedMutableCaptures);
