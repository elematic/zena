...
suite('codegen: inline tuples', () => {
  suite('multi-value returns', () => {
    ...
    test('function returning (i32, i32, i32)', async () => {
      const source = `
        export let triple = (): inline (i32, i32, i32) => ((1, 2, 3));
      `;
      const exports = await compileAndInstantiate(source);
    ...
    test('function returning computed values', async () => {
      const source = `
        export let compute = (a: i32, b: i32): inline (i32, i32) => ((a + b, a * b));
      `;
      const exports = await compileAndInstantiate(source);
    ...
    test('return statement in block body', async () => {
      const source = `
        export let pair = (): inline (i32, i32) => {
          return (1, 2);
        };
      `;
      const exports = await compileAndInstantiate(source);
    ...
    test('conditional multi-value return', async () => {
      const source = `
        export let choose = (flag: boolean): inline (i32, i32) => {
          if (flag) {
            return (10, 20);
          } else {
            return (30, 40);
  ...
  suite('destructuring', () => {
    ...
    test('multiple destructurings', async () => {
      const source = `
        let pair = (x: i32): inline (i32, i32) => ((x, x * 2));
  ...
  suite('mixed types', () => {
    test('(i32, boolean)', async () => {
      const source = `
        export let check = (n: i32): inline (i32, boolean) => {
          return (n * 2, n > 5);
        };
      `;
      const exports = await compileAndInstantiate(source);
    ...
    test('destructuring mixed types', async () => {
      const source = `
        let check = (n: i32): inline (i32, boolean) => ((n * 2, n > 5));
    ...
    test('(f64, f64)', async () => {
      const source = `
        export let sincos = (angle: f64): inline (f64, f64) => {
