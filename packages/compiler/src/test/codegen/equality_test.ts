...
suite('CodeGenerator - Equality', () => {
  ...
  async function runEqualityTest(body: string, setup: string = ''): Promise<number> {
    const source = `
      ${setup}
      export let main = (): i32 => {
        ${body}
  ...
  test('string equality (constructed)', async () => {
    ...
    assert.strictEqual(await runEqualityTest(check('("he" + "llo") == "hello"')), 1);
    assert.strictEqual(await runEqualityTest(check('("he" + "llo") == ("he" + "llo")')), 1);
  });
  ...
  test('class reference equality', async () => {
    ...
    assert.strictEqual(await runEqualityTest(`
