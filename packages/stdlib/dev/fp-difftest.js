...
addPrint(Number.MAX_VALUE);
addPrint(Number.MIN_VALUE);
...
for (let i = 0; i < Math.min(N, 30000); i++)
  addF32(Number(rand64() & 0x7fffffffn));
for (let e = 0; e < 255; e++) {
  addF32((e << 23) >>> 0);
  addF32(((e << 23) | 1) >>> 0);
...
writeFileSync(
  `${root}/scratch/fp-corpus-print.txt`,
  printCases.map((c) => c[0]).join('\n') + '\n',
);
writeFileSync(
  `${root}/scratch/fp-corpus-parse.txt`,
  parseCases.map((c) => c[0]).join('\n') + '\n',
);
writeFileSync(
  `${root}/scratch/fp-corpus-f32.txt`,
  f32Cases.map((c) => c[0]).join('\n') + '\n',
);
...
console.log(
  `corpus: ${printCases.length} print, ${parseCases.length} parse, ${f32Cases.length} f32`,
);
...
const actual = (await import('node:fs'))
  .readFileSync(`${root}/scratch/fp-actual.txt`, 'utf8')
  .split('\n');
...
const report = (kind, input, got, want) => {
  ...
  if (++failures <= 25)
    console.log(`  FAIL ${kind} ${input}: got ${got}, want ${want}`);
};
