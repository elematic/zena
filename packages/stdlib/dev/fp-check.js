...
// --- the exponent estimates -------------------------------------------------
{
  ...
  for (let p = -400; p <= 400; p++) {
    const want = BigInt(p) >= 0n ? BigInt(bitLen(10n ** BigInt(p)) - 1) : null;
  ...
  console.log(
    bad === 0 ? 'log2Pow10: exact over [-400,400]' : `log2Pow10: ${bad} wrong`,
  );
  if (bad) failures += bad;
}
{
  ...
  console.log(
    bad === 0 ? 'log10Pow2: exact over [0,1100]' : `log10Pow2: ${bad} wrong`,
  );
  if (bad) failures += bad;
}
  ...
  for (let p = -344; p <= 324; p += 1) {
    for (let t = 0; t < 40; t++) {
      ...
      if (got !== want) {
        ...
        if (bad <= 3)
          console.log(
            `  uscale p=${p} x=${x.toString(16)} got ${got.toString(16)} want ${want.toString(16)}`,
          );
      }
    }
  }
...
// --- parse ------------------------------------------------------------------
{
  ...
  for (const [d, p] of cases) {
    ...
    if (!Number.isFinite(want) && Math.abs(p) < 400) {
      /* still compare */
    }
    n++;
    let got;
    try {
      ...
      fail(`parse(${text}) threw ${err.message}`);
      bad++;
      continue;
    }
  ...
  console.log(
    bad === 0 ? `parse: ${n}/${n} match Number()` : `parse: ${bad}/${n} WRONG`,
  );
  failures += bad;
}
...
// --- short ------------------------------------------------------------------
{
  ...
  const randDouble = () => {
    ...
    do {
      for (let k = 0; k < 8; k++)
        rnd.setUint8(k, Math.floor(Math.random() * 256));
      v = rnd.getFloat64(0);
  ...
  for (const v of vals) {
    ...
    try {
      ...
      fail(`short(${v}) threw ${err.message}`);
      bad++;
      continue;
    }
    ...
    if (back !== v) {
      bad++;
      if (bad <= 10)
        console.log(`  short(${v}) = ${d}e${p} -> ${back} (no round-trip)`);
      continue;
    }
    ...
    if (ourDigits > jsDigits) {
      ...
      if (bad <= 10)
        console.log(
          `  short(${v}) = ${d}e${p}: ${ourDigits} digits, JS uses ${jsDigits}`,
        );
    }
  }
  console.log(
    bad === 0
      ? `short: ${n}/${n} round-trip and are shortest`
      : `short: ${bad}/${n} WRONG`,
  );
  failures += bad;
}
