...
export function recover(p) {
  ...
  if (restHi !== 0n || restLo !== 0n) {
    ...
    if (pmLo === 0n) {
      pmHi = (pmHi + 1n) & U64;
      if (pmHi === 0n)
        throw new Error(`recover(${p}): round-up overflowed 128 bits`);
    }
  }
  // Borrow form.
...
export function verify() {
  ...
  for (let p = P_MIN; p <= P_MAX; p++) {
    ...
    if (gh !== hi || gl !== lo) {
      if (++bad <= 10) {
        console.log(
          `  p=${p}: recovered ${gh.toString(16)}:${gl.toString(16)}, ` +
...
export function emit() {
  const out = [];
  out.push(
    `// GENERATED by packages/stdlib/dev/fp-gen-tables.js — do not edit by hand.`,
  );
  out.push(`//`);
  out.push(`// The compressed power-of-ten cache. A full 128-bit table over`);
  out.push(
    `// p in [${P_MIN}, ${P_MAX}] would be ${(P_MAX - P_MIN + 1) * 16} bytes; these two hold`,
  );
  out.push(
    `// ${(Q_MAX - Q_MIN + 1) * 24 + STRIDE * 8} and rebuild any entry with one 192x64 multiply.`,
  );
  out.push(
  ...
  out.push('');
  out.push(
    `/** 10^(${STRIDE}q) truncated to 192 bits, three words high to low, q from ${Q_MIN}. */`,
  );
  ...
  out.push('');
  out.push(
    `/** 5^r normalized to bit 63 (exact: 5^${STRIDE - 1} < 2^61), r in [0, ${STRIDE - 1}]. */`,
  );
