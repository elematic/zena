...
- **Generic Methods on VTables (Type Erasure)**: For generic methods on classes and interfaces (e.g. `map<U>`), the current compilers simplify vtable generation by erasing the method's generic type parameters to `anyref` inside the vtable slot.
  ...
  - **Virtual/Interface Dispatch**: NOT IMPLEMENTED (status corrected 2026-07-26). The erased copies (`Array_i32.map` over `anyref`) exist and occupy _class_-vtable slots, but no boxing or unboxing adapters were ever generated, generic methods get no _interface_ vtable slot at all, and both compilers crashed with internal errors on a dispatch site like `(s: Array<i32>).map(f)`. Both checkers now reject it with a NotCallable diagnostic instead (see [#116](https://github.com/elematic/zena/issues/116); semantics test `interfaces/generic-method-virtual-dispatch.zena`). The diagnostic is a stopgap until §"Generic interface dispatch" below is implemented.
