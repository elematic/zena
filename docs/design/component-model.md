...
| WIT-typed modules (stage 1)             | 🚧 Growing — flat scalars; `async`-without-result (functions returning `Future<void>`); `string`/`list<u8>` params, aggregates both ways — results lifted at canonical offsets, parameters lowered into flat core slots (staged buffers arena-tracked, one `releaseStaged()` per call; variant payload joins across core types still refuse) — real WIT spliced into the encoder; resources as Disposable handle classes; async imports with results (async wrappers awaiting the driver, results lifted from the trailing return area into `Outcome`); `stream<u8>` in any position as `zena:stream`'s `Stream<u8>`, pumped across the canonical ends by `zena:component-stream` on the driver's event dispatch; `future<T>` in any position as a `Future`, moved by generated per-payload-type helpers whose canon builtins are declared at the payload's own component type (`future:<interface>#<payload>`, alias-encoded against the imported instance), with `result` payloads riding `Outcome` and bare result arms — anywhere — as `void` lanes; parameters past the canonical limit (sixteen core values on a synchronous call, four on an asynchronous one) lowered into one staged buffer and passed as its address |
...
| `resource`                          | `final class` + `Disposable`              | **25**            | ✅ synthesized              |
