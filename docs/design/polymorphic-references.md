...
* **Why they are needed**: Wasm GC only supports single linear inheritance.
  ...
  * **Records** (Structural): While simple prefix extensions (e.g., passing `{a: i32, b: i32}` to a variable expecting `{a: i32}`) *can* map to Wasm's width subtyping without fat pointers, arbitrary intersections (e.g., casting `{a: i32, b: i32, c: i32}` to `{b: i32}`) break the linear inheritance constraint and require an adapter.
...
* **Generation**: Fat pointers are manufactured *on demand* at the moment of upcasting (e.g., assigning a class to an interface variable, or adapting a record to a non-prefix target).
...
* **Future SROA Heuristics**: If we ever implement formal SROA to eliminate the initial `struct.new` heap allocations, it *must* be limited to local variables only. We could introduce heuristics (e.g., if a fat pointer local is accessed more than $N$ times, or never escapes to the heap) to fully unpack it into two separate unboxed locals `$inst` and `$vtable`, skipping the struct allocation entirely. But currently, typed scratch locals provide the performance baseline we need.
...
```
At the call site, we read the typed function reference and invoke it directly. The only `ref.cast` in the entire lifecycle occurs *inside* the generated getter function, which downcasts the `anyref` back to the concrete backing struct before reading memory.
